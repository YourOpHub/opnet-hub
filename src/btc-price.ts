...
interface BinanceTicker { price?: string }
interface BinanceStats { priceChangePercent?: string }
interface KrakenResult { result?: Record<string, { c?: string[]; o?: string }> }
...
async function tryBinance(): Promise<PriceData | null> {
  try {
    ...
    const ticker = (await tickerRes.json()) as BinanceTicker;
    const price = parseFloat(ticker.price ?? '0');
    if (!price) return null;
    let change = 0;
    if (statsRes.ok) { const s = (await statsRes.json()) as BinanceStats; change = parseFloat(s.priceChangePercent ?? '0') || 0; }
...
async function tryKraken(): Promise<PriceData | null> {
  try {
    ...
    const d = (await res.json()) as KrakenResult;
    const pair = d.result?.XXBTZUSD ?? d.result?.XBTUSD;
    if (pair?.c?.[0] == null) return null;
    const price = parseFloat(pair.c[0]);
    const open = parseFloat(pair.o ?? '0') || price;
