...
import { TESTNET_CONTRACTS, DEPLOYER_MLDSA_HEX, DEPLOYER_TWEAKED_HEX, getContractOpscanUrl } from '../contracts';

/** Simulated AMM liquidity pool for MINE/VIBE on testnet.
 *  Constant-product formula: x * y = k
 *  Initial reserves seeded from deployer's token supply. */
...
const SwapUI: React.FC<{ walletAddress?: string }> = ({ walletAddress }) => {
  ...
  const [swapResult, setSwapResult] = useState<{ type: 'success' | 'demo'; hash?: string; amtOut?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (!walletAddress || !net) { setBalances({}); return; }
    opnet.setNetwork(net);
    setBalLoading(true);
    const jobs: Promise<void>[] = [];
    for (const [sym, tok] of Object.entries(TESTNET_CONTRACTS)) {
      jobs.push(
        opnet.getTokenBalance(tok.address, DEPLOYER_MLDSA_HEX, DEPLOYER_TWEAKED_HEX)
    ...
    <div>
      <div className="Pg" style={{ marginBottom: 14, textAlign: 'center', padding: '24px 18px' }}>
        ...
        <div style={{ color: 'var(--t3)', fontSize: '.8rem', maxWidth: 440, margin: '0 auto' }}>
          Swap MINE ↔ VIBE tokens on Bitcoin L1 using our test liquidity pool. Constant-product AMM (x·y=k).
        </div>
      </div>
      ...
      <div style={{ maxWidth: 440, margin: '0 auto' }}>
        <div className="P" style={{ padding: 20, position: 'relative' }}>
          ...
          <div style={{ padding: '14px', background: 'rgba(255,255,255,.03)', borderRadius: 'var(--rad)', border: '1px solid var(--bd)', marginBottom: 4 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <span style={{ fontSize: '.65rem', color: 'var(--t4)' }}>
                Balance: {fmtBal(fromBal, from.decimals)}
              </span>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input
                type="text" inputMode="decimal" value={fromAmt}
          ...
          >
            {swapping ? '🔄 Swapping…' : !hasPool ? 'No pool for this pair' : fromVal > 0 ? `Swap ${from.symbol} → ${to.symbol}` : 'Enter an amount'}
          </button>
          ...
          {swapResult && (
            <div style={{ marginTop: 12, padding: '10px 12px', background: 'var(--gG)', border: '1px solid var(--gB)', borderRadius: 'var(--rad)', fontSize: '.72rem' }}>
              {swapResult.type === 'success' && (
                <><div style={{ color: 'var(--g)', fontWeight: 700, marginBottom: 4 }}>✓ Swap Executed</div>
                <div style={{ color: 'var(--t2)', fontSize: '.7rem' }}>Received: {swapResult.amtOut} {to.symbol}</div>
                <div style={{ fontFamily: 'var(--fm)', color: 'var(--t3)', wordBreak: 'break-all', fontSize: '.58rem', marginTop: 4 }}>tx: {swapResult.hash?.slice(0, 20)}…{swapResult.hash?.slice(-8)}</div>
              ...
              {swapResult.type === 'demo' && (
                <><div style={{ color: 'var(--y)', fontWeight: 700, marginBottom: 4 }}>⚡ Simulated Swap — Connect wallet for real execution</div>
            ...
            return (
              <div key={tok.symbol} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <a href={getContractOpscanUrl(tok.address)} target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: '.6rem', color: 'var(--c2)', whiteSpace: 'nowrap', textDecoration: 'none', flexShrink: 0 }}>OPScan ↗</a>
              </div>
            );
          })}
        </div>
        ...
        <div className="P" style={{ marginTop: 14, padding: 16, fontSize: '.75rem', color: 'var(--t3)', lineHeight: 1.5 }}>
          <div className="Lb">💧 Test Liquidity Pool</div>
          <p>This swap uses a simulated constant-product AMM pool (x·y=k) for the MINE/VIBE pair. The pool is pre-seeded with test reserves to demonstrate the swap mechanics.</p>
